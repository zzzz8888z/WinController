...
if (typeof Worker === "undefined" || (typeof location !== "undefined" && location.protocol === "file:")) {
    ...
    /// Is this a browser?
        ...
        (function () {
            ...
            function non_worker_lzma(path) {
                ...
                fake_lzma = {
                    ...
                    lzma2_compress: function lzma2_compress(mixed, mode, on_finish, on_progress) {
                        if (global_var.LZMA_WORKER) {
                            global_var.LZMA_WORKER.lzma2_compress(mixed, mode, on_finish, on_progress);
                        } else {
                            /// Wait
                            setTimeout(function ()
                            {
                                fake_lzma.lzma2_compress(mixed, mode, on_finish, on_progress);
                            }, 50);
                        }
                    },
                    lzma2_decompress: function lzma2_decompress(byte_arr, on_finish, on_progress) {
                        if (global_var.LZMA_WORKER) {
                            global_var.LZMA_WORKER.lzma2_decompress(byte_arr, on_finish, on_progress);
                        } else {
                            /// Wait
                            setTimeout(function ()
                            {
                                fake_lzma.lzma2_decompress(byte_arr, on_finish, on_progress);
                            }, 50);
                        }
                    },
        ...
        console.error("Can't load the LZMA2 worker. Sorry.");
    }
} else {
    ...
    this.LZMA = function (lzma_path) {
        ...
        return (function () {
            ...
            return {
                ...
                lzma2_compress: function lzma2_compress(mixed, mode, on_finish, on_progress) {
                    send_to_worker(action_lzma2_compress, mixed, mode, on_finish, on_progress);
                },
                lzma2_decompress: function lzma2_decompress(byte_arr, on_finish, on_progress) {
                    send_to_worker(action_lzma2_decompress, byte_arr, false, on_finish, on_progress);
                },
                worker: function worker() {
                    return lzma_worker;
                }
